...
- **Current phase:** Phase 0 done; Phase 1 in progress (parser landed)
- **Overall status:** 🟢 Live data path proven; raw `/api/state` parser + unit test **passing**
...
- **Build/test env works** (deps in `deps/build/`, toolchain installed). Verified:
...
- **Heads-up for next session:** a large *parallel* multiACE implementation exists
  on sibling branches (`validation/pr30-fix-rest` ~273 commits, `feat/wire-multiace-printer-lifecycle`,
  ...
  reuse non-conflicting pieces from those branches. See Decisions log.
