...
class Bridge:
    ...
    def _set_ace_plan(self, seqid, params, event_id):
        ...
        bridge has no rewriter and no logical file: it reads a FINISHED gcode and parses
        the plan out of its header. It could permute that parsed plan and hand it back,
        and the page would look right - which is the reason not to. A stand-in that
        pretends to have done the work turns "the page and the printer agree" into "the
        page and my stub agree", and the difference would surface on the machine.
