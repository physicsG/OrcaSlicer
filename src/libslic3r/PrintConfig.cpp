...
void PrintConfigDef::init_fff_params()
{
    ...
    */
    ...
    // Per-plate, not per-printer: it belongs to the plate the user assigned, and rides in the
    // 3MF beside it (see PartPlate::set_ace_plan_layout). One head index per filament, in
    // filament order; slots are not stored because evaluate_assignment derives them from the
    // print's own first-use order. Empty means "no layout applied - use the planner".
    // Deliberately not in any preset list: a hand layout is project data, not a setting.
    def = this->add("ace_plan_layout", coInts);
    def->label = L("multiACE filament layout");
    def->tooltip = L("The toolhead each filament is loaded on, as applied in the filament assignment "
                     "dialog. Empty when the plan is left to the optimiser.");
    def->mode = comDevelop;
    def->min = -1;
...
void DynamicPrintConfig::set_num_extruders(unsigned int num_extruders)
{
    ...
    for (const std::string &key : print_config_def.extruder_option_keys()) {
        ...
        if (opt == nullptr) {
            // Config predates this option (e.g. a project saved before it existed):
            // seed it from the default instead of asserting, as the filament path does.
            const ConfigOption *default_opt = defaults.option(key);
            if (default_opt == nullptr)
                continue;
            this->set_key_value(key, default_opt->clone());
...
// change_filament_gcode
    ...
    new_def("ace_head", coInt, "ACE head", "multiACE: toolhead the incoming filament lives on (0-based; -1 = no plan)");
    new_def("ace_slot", coInt, "ACE slot", "multiACE: slot of the incoming filament within its head (0-based; -1 = no plan)");
    new_def("ace_unit", coInt, "ACE unit", "multiACE: unit feeding the incoming filament's head (0-based; -1 = none)");
